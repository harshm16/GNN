        ...
        if x.shape[0] % batch_size == 0:
            if adj_matrix.shape[1] == x.shape[0]:
                ...
                for i in range(iterations):
                    ...
                    K_epsilon_iter = torch.cdist(normalized_random_x.reshape(batch_size,x.shape[0]//batch_size, x.shape[1]),normalized_random_x.reshape(batch_size,x.shape[0]//batch_size, x.shape[1])).mean(dim=(2,3))
